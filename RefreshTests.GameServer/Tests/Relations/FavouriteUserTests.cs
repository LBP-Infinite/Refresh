...
public class FavouriteUserTests : GameServerTest
{
    ...
    public void FavouriteAndUnfavouriteUser()
    {
        ...
        //Favourite a user
        HttpResponseMessage message = client.PostAsync($"/LITTLEBIGPLANETPS3_XML/favourite/user/{user2.Username}", new ReadOnlyMemoryContent(Array.Empty<byte>())).Result;
        Assert.That(message.StatusCode, Is.EqualTo(OK));
        
        //Get the favourite users now
        message = client.GetAsync($"/LITTLEBIGPLANETPS3_XML/favouriteUsers/{user1.Username}").Result;
        Assert.That(message.StatusCode, Is.EqualTo(OK));
        ...
        message = client.PostAsync($"/LITTLEBIGPLANETPS3_XML/unfavourite/user/{user2.Username}", new ReadOnlyMemoryContent(Array.Empty<byte>())).Result;
        Assert.That(message.StatusCode, Is.EqualTo(OK));
    ...
    public void CantFavouriteMissingUser(bool psp)
    {
        ...
        HttpResponseMessage message = client.PostAsync($"/LITTLEBIGPLANETPS3_XML/favourite/user/pain peko", new ReadOnlyMemoryContent(Array.Empty<byte>())).Result;
        Assert.That(message.StatusCode, Is.EqualTo(psp ? OK : NotFound));
        
        //Get the favourite users
        message = client.GetAsync($"/LITTLEBIGPLANETPS3_XML/favouriteUsers/{user.Username}").Result;
        Assert.That(message.StatusCode, Is.EqualTo(OK));
    ...
    public void CantUnfavouriteMissingUser(bool psp)
    {
        ...
        HttpResponseMessage message = client.PostAsync($"/LITTLEBIGPLANETPS3_XML/unfavourite/user/womp womp", new ReadOnlyMemoryContent(Array.Empty<byte>())).Result;
        Assert.That(message.StatusCode, Is.EqualTo(psp ? OK : NotFound));
    }
    ...
    public void CantFavouriteUserTwice(bool psp)
    {
        ...
        message = client.PostAsync($"/LITTLEBIGPLANETPS3_XML/favourite/user/{user2.Username}", new ReadOnlyMemoryContent(Array.Empty<byte>())).Result;
    ...
    public void CantUnfavouriteUserTwice(bool psp)
    {
        ...
        HttpResponseMessage message = client.PostAsync($"/LITTLEBIGPLANETPS3_XML/unfavourite/user/{user2.Username}", new ReadOnlyMemoryContent(Array.Empty<byte>())).Result;
        Assert.That(message.StatusCode, Is.EqualTo(psp ? OK : Unauthorized));
    }
    ...
    public void CantGetFavouriteUsersForInvalidUser()
    {
        ...
        HttpResponseMessage message = client.GetAsync("/LITTLEBIGPLANETPS3_XML/favouriteUsers/WHO ARE YOU?").Result;
